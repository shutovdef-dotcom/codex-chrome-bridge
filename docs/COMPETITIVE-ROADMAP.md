...
3. Confirm the smoke output reports `coverage.ok: true` for `workspace`, `setWorkspace` strict policy, `session-summary`, default `debug-bundle`, `observe`, `find-elements` including nearby text, `extract`, `pdf`, strict outside-tab blocking, and cleanup metadata.
