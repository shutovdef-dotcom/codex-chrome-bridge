...
Prompt cleanup uses the same saved-group feature detection and ungroup-before-close path as `close-tab` and `close-group`, so short-lived prompt tabs do not leave behind new saved closed `Codex Bridge` groups when Chrome cleanup runs. The background service worker also sweeps existing managed groups on startup and registers tab-group create/update listeners that re-apply the best-effort unsaved marker to managed Codex groups when Chrome exposes that surface. Managed detection includes `Codex Bridge ...` session titles plus bridge-created workspace titles remembered in extension-local storage.
