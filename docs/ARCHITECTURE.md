...
- `tab-group-persistence.js` owns feature-detected saved-tab-group disablement for current no-op/future Chrome API support, plus startup sweeping, create/update/removal listeners, and managed tab membership tracking that keep Codex groups ephemeral when that surface exists. It treats default, `Codex Bridge ...` session, and remembered bridge-created workspace titles as managed.
