...
Coverage-plan and live skipped or failed outputs include `verification.finalCommands` and `verification.finalMcpCalls`, so CLI and MCP agents can recover from stale bridge or extension versions without guessing the final reload, live doctor, and smoke sequence.
...
| `chrome_bridge_codex_config` | `codex-config` | read | 5000 ms | no | no | Print a Codex MCP configuration snippet using the current Node executable. |
...
| `chrome_bridge_snapshot` | `snapshot` | read | 30000 ms | no | yes | Read a bounded structured page snapshot with querySelector-verified element selectors. |
