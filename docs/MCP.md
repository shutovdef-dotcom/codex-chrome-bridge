...
If two Chrome profiles have the extension enabled at the same time, the local bridge accepts both connections and reports them in `chrome_bridge_health` under `extensions[]`. Copy the desired `extensions[].info.profileId` or `extensions[].info.clientId` into the MCP process environment as `CHROME_BRIDGE_PROFILE_ID`.

Run separate MCP server processes for separate profiles, each with its own `CHROME_BRIDGE_PROFILE_ID`. When multiple profiles are connected and a process does not set that variable, browser commands fail with `AMBIGUOUS_EXTENSION_PROFILE` so one agent cannot accidentally control the other Chrome profile.
