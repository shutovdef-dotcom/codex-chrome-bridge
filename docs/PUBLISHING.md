...
`check:runtime-smoke-plan` runs the offline smoke plan against a dead bridge URL, verifies stale bridge-server and stale-extension skip metadata, structured JSON output, `finalVerificationComplete: false`, and nonzero CLI-exit preservation against fake `/health` servers, and fails if `--coverage-plan` starts contacting the live bridge.
