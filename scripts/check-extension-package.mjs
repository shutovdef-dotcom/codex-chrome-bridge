...
async function runBuild(outputPath) {
  try {
    const result = await execFileAsync(process.execPath, [
      path.join(rootDir, 'scripts/build-extension-zip.mjs'),
      '--out',
      outputPath,
    ], {
      cwd: rootDir,
...
check(packageJson.scripts?.['extension:zip'] === 'node ./scripts/build-extension-zip.mjs', 'package scripts must expose extension:zip');
check(packageJson.scripts?.['check:extension-package'] === 'node ./scripts/check-extension-package.mjs', 'package scripts must expose check:extension-package');
