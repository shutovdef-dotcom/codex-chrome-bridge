...
async function checkSourceSurface() {
  ...
  check(packageJson.scripts?.['check:ubs-fixes'] === 'node ./scripts/check-ubs-fixes.mjs', 'package.json must expose check:ubs-fixes');
  check(packageJson.scripts?.check?.includes('node --check ./scripts/check-ubs-fixes.mjs'), 'npm run check must syntax-check check-ubs-fixes');
