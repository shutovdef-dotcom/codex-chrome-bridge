...
- [ ] `npm run check:audit`
- [ ] `npm run check:pack`
- [ ] Live upgrade/smoke, only if browser behavior changed and no other session is using the bridge: `chrome-bridge reload-extension --confirm`, `chrome-bridge doctor --live-checks`, then `npm run runtime-smoke`; use top-level `nextCommand` / `nextAction` for skipped or failed smoke recovery, with nested `verification.nextCommand` / `verification.nextAction` carrying the same context; require `ok: true`, `coverage.ok: true`, and `verification.status: "passed"`
